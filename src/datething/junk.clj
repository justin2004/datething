...
(defn get-query-execution [query]
  (let [query-execution (org.apache.jena.query.QueryExecutionFactory/create
                         query
                                 ; (format nil "~A ~A" prefixes query)
                         model)]
    query-execution))

(do  (def rs (.execSelect (get-query-execution "PREFIX f: <java:datething.> 
                                            select * where { bind(now() as ?now)
                                                             bind(12 as ?num)
                                                             bind(f:core(?now) as ?bbb)}")))
     (print (.next rs)))
